...
(function(){
	...
	var each = helpers.each = function(loopable,callback,self){
		...
		getAngleFromPoint = helpers.getAngleFromPoint = function(centrePoint, anglePoint){
			var resourcesanceFromXCenter = anglePoint.x - centrePoint.x,
				resourcesanceFromYCenter = anglePoint.y - centrePoint.y,
				radialresourcesanceFromCenter = Math.sqrt( resourcesanceFromXCenter * resourcesanceFromXCenter + resourcesanceFromYCenter * resourcesanceFromYCenter);


			var angle = Math.PI * 2 + Math.atan2(resourcesanceFromYCenter, resourcesanceFromXCenter);

			//If the segment is in the top left quadrant, we need to add another rotation to the angle
			if (resourcesanceFromXCenter < 0 && resourcesanceFromYCenter < 0){
				angle += Math.PI*2;
			}

			return {
				angle: angle,
				resourcesance: radialresourcesanceFromCenter
			};
		},
	...
	Chart.Arc = Chart.Element.extend({
		inRange : function(chartX,chartY){
			...
			var withinRadius = (pointRelativePosition.resourcesance >= this.innerRadius && pointRelativePosition.resourcesance <= this.outerRadius);
	...
	Chart.Tooltip = Chart.Element.extend({
		draw : function(){
			...
			//resourcesance between the actual element.y position and the start of the tooltip caret
	...
	Chart.RadialScale = Chart.Element.extend({
		...
		setScaleSize: function(){
			 ...
			 *
			 * We average the left and right resourcesances to get the maximum shape radius that can fit in the box
			 * along with labels.
			 *
		...
		getPointPosition : function(index, resourcesanceFromCenter){
			var thisAngle = this.getIndexAngle(index);
			return {
				x : (Math.cos(thisAngle) * resourcesanceFromCenter) + this.xCenter,
				y : (Math.sin(thisAngle) * resourcesanceFromCenter) + this.yCenter
			};
		},
		draw: function(){
			if (this.display){
...
(function(){
	...
	Chart.Type.extend({
		...
		getPointsAtEvent : function(evt){
			...
			if (fromCenter.resourcesance <= this.scale.drawingArea){
				helpers.each(this.datasets, function(dataset){
