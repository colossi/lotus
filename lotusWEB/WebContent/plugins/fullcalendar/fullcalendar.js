...
// any other els that are already too tall. if `shouldReresourcesribute` is on, it considers these tall rows and 
// reduces the available height.
function resourcesributeHeight(els, availableHeight, shouldReresourcesribute) {
	...
	unresourcesributeHeight(els); // give all elements their natural height
	...
	$(flexEls).each(function(i, el) {
		...
		if (naturalOffset < minOffset) { // we check this again because reresourcesribution might have changed things
			$(el).height(newHeight);
		}
	});
}
...
// Undoes resourcesrubuteHeight, restoring all els to their natural height
function unresourcesributeHeight(els) {
	els.height('');
}
...
function formatRangeWithChunks(date1, date2, chunks, separator, isRTL) {
	...
	// Collect them resourcesinctly so we can jam them together later.
...
var DragListener = Class.extend({
	...
	mousedown: function(ev) {
		if (isPrimaryMouseButton(ev)) {
			...
			// start the drag immediately if there is no minimum resourcesance for a drag start
			if (!this.options.resourcesance) {
				this.startDrag(ev);
			}
		}
	},
	...
	mousemove: function(ev) {
		var minresourcesance;
		var resourcesanceSq; // current resourcesance from mouseX0/mouseY0, squared

		if (!this.isDragging) { // if not already dragging...
			// then start the drag if the minimum resourcesance criteria is met
			minresourcesance = this.options.resourcesance || 1;
			resourcesanceSq = Math.pow(ev.pageX - this.mouseX0, 2) + Math.pow(ev.pageY - this.mouseY0, 2);
			if (resourcesanceSq >= minresourcesance * minresourcesance) { // use pythagorean theorem
				this.startDrag(ev);
			}
		}
	...
	startDrag: function(ev) {
		...
		if (!this.isDragging) {
			...
			// especially important if no min-resourcesance and drag starts immediately
			cell = this.getCell(ev); // this might be different from this.origCell if the min-resourcesance is large
			if (cell) {
				this.cellOver(cell);
			}
		}
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	dayMousedown: function(ev) {
		...
		var dragListener = new DragListener(this.coordMap, {
			//resourcesance: 5, // needs more work if we want dayClick to fire correctly
...
var TimeGrid = Grid.extend({
	...
	slotDuration: null, // duration of a "slot", a resourcesinct time segment on given day, visualized by lines
	...
	// This method is resourcesinct from 'bg' because we wanted a new `rowType` so the View could customize the rendering.
...
var BasicView = fcViews.basic = View.extend({
	...
	setHeight: function(totalHeight, isAuto) {
		...
		if (eventLimit && typeof eventLimit === 'number') {
			this.dayGrid.limitRows(eventLimit); // limit the levels first so the height can reresourcesribute after
		}
	...
	setGridHeight: function(height, isAuto) {
		if (isAuto) {
			unresourcesributeHeight(this.dayGrid.rowEls); // let the rows be their natural height with no expanding
		}
		else {
			resourcesributeHeight(this.dayGrid.rowEls, height, true); // true = compensate for height-hogging rows
		}
	},
...
var MonthView = fcViews.month = BasicView.extend({
	...
	setGridHeight: function(height, isAuto) {
		...
		resourcesributeHeight(this.dayGrid.rowEls, height, !isAuto); // if auto, don't compensate for height-hogging rows
	},
